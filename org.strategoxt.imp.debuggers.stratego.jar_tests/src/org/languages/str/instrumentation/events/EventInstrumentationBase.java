...
public abstract class EventInstrumentationBase {
	...
	private void loadDSLProgramSource(String dslProgramLocation)
	{
		...
		Assert.assertTrue("Could not parse the Stratego script", b);
	 ...
	 */
	protected void compareDslProgramSourceWithAlng(String program_location)
	{
		...
		i.setCurrent(input);
		HybridInterpreterHelper.safeInvoke(i, StrTestConstants.PARSE_STRATEG_SUGAR_FILE);
		Assert.assertTrue(b);
		Assert.assertEquals(i.current().toString(), source.toString());
	}
}
